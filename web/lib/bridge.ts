 ...
 *
 * All requests are POSTed JSON with no auth (the bridge only listens on
 * loopback). Errors are surfaced as thrown rejections so the UI can show
 * meaningful state.
 */
...
export async function connect(ip: string): Promise<boolean> {
